...
import 'package:frist_pages/dataLayer/model/shopingItems.dart';
...
Future<User>getuserByID(int userId) async{
  ...
  List<Map> data=await d!.query(users_table , where:'user_id=?', whereArgs: [userId] );
  Map item=data[0];
  User u=User(item['user_id'],item['user_name'], item['password'], item['email'] , item['phone'] , item['address']);
  return u;
}
 ...
 ///////////////////////////// product function //////////////////////////////////
  ...
  Future<int> deleteProductByName(String name) async {
    Database? d=await db;
...
Future<Favorite?> addToFavorites(Favorite f) async {
  ...
  List<Map> result = await dbc!.query(
    favorites_table,
    where: 'user_id = ? AND product_id = ?',
    whereArgs: [f.u_id, f.p_id],
  );
  ...
  int id = await dbc.insert(
    favorites_table,
    {'favorite_id': f.id, 'user_id': f.u_id, 'product_id': f.p_id},
  );
...
Future<List<cartItem>> showCart(int userId) async {
  Database? d = await db;
